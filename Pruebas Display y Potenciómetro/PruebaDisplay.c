...
int main(void)
{
  ...
  // Inicialización básica del chip
    TM1637_Start();
    TM1637_WriteByte(0x40); // Modo datos
    TM1637_Stop();
  ...
  while (1)
  {
	  // 1. Enviar los datos (8888)
	        TM1637_Start();
	        TM1637_WriteByte(0xC0);
	        for(int i = 0; i < 4; i++) TM1637_WriteByte(0xFF);
	        TM1637_Stop();
	        ...
	        // 3. Apagar Display (Parpadeo)
	        TM1637_SetBrightness(7, 0);
	        HAL_Delay(500);
...
void TM1637_Delay(void) {
    for(volatile int i = 0; i < 100; i++); // Reducido para no ralentizar demasiado el F411
}
